...
export default function DashboardPage() {
  ...
  const allItems = data?.topAttentionItems || [];
  const filteredItems = allItems.filter((item: any) => {
    ...
    if (selectedFilter === 'ASSIGNED') return item.type === 'ASSIGNED';
    return true;
  });
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-emerald-800 text-white shadow-xs border border-emerald-700">
            ...
            {selectedFilter && (
              <button
                ...
                className="self-start md:self-center px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-emerald-800 hover:bg-emerald-700 text-white transition border border-emerald-600"
              >
                Clear Filter
              </button>
            )}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                ...
                {selectedFilter && (
                  <span className="text-xs font-semibold text-emerald-700">({selectedFilter})</span>
                )}
              </h3>
